...
class Text:
    ...
    vendorID = "Vendor ID "
    timeOut = "Time between key presses: "
...
class HID(eg.PluginClass):
    ...
    def ButtonCallback(self, data):
        global CAN_REPEAT_AGAIN_TIME
        if len(data):
            #one or more buttons pressed
            btnPressed = []
            if CAN_REPEAT_AGAIN_TIME < time.clock(): 
                for num in data:
                ...
                CAN_REPEAT_AGAIN_TIME = time.clock() + self.timeOut
    ...
    def Configure(self,
        ...
        #timeout interval control
        optionsSizer.Add(
            wx.StaticText(panel, -1, Text.timeOut),
            (6, 0),
            flag = wx.ALIGN_CENTER_VERTICAL)
        timeOutControl = eg.SpinNumCtrl(panel, -1, timeOut)
        optionsSizer.Add(timeOutControl, (6, 1), (1,4))
